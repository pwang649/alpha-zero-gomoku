...
class Leaner():
    ...
    def self_play(self, first_color, libtorch, show):
        ...
        player1 = MCTS(libtorch, self.c_puct,
                    self.num_mcts_sims, self.action_size)
        player2 = MCTS(libtorch, self.c_puct,
            self.num_mcts_sims, self.action_size)
    ...
    def _contest(self, network1, network2, first_player, show):
        # create MCTS
        player1 = MCTS(network1, self.c_puct,
            self.num_mcts_sims, self.action_size)
        player2 = MCTS(network2, self.c_puct,
                    self.num_mcts_sims, self.action_size)
    ...
    def play_with_human(self, human_first=True, checkpoint_name="best_checkpoint"):
        ...
        mcts_best = MCTS(libtorch_best, \
             self.c_puct, self.num_mcts_sims * 6, self.action_size)
